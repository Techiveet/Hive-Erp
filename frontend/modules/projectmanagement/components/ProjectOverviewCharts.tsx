...
interface ExpertiseMetric {
  name: string;
  [key: string]: string | number;
}

interface RiskAnomalyMetric {
  name: string;
  risk: number;
  anomaly: number;
  load: number;
...
interface FinancialForecastMetric {
  date: string;
  actual?: number;
  forecast: number;
  budget: number;
...
interface TimelineScenarioMetric {
  date: string;
  actual?: number;
  baseline: number;
  optimistic: number;
...
interface ProfitabilityScenarioMetric {
  date: string;
  revenue: number;
...
interface QualityVelocityMetric {
  velocity: number;
  bugRate: number;
  name: string;
}
...
export function ProjectOverviewCharts({ project, tasks }: ProjectOverviewChartsProps) {
  ...
  const healthMetrics = useMemo<HealthMetric[]>(() => {
    ...
    if (project.budget && project.budget > 0) {
      ...
      const estimatedCost = (totalMinutes / 60) * (project.hourly_rate || 50);
      budgetUtilization = Math.min(100, (estimatedCost / project.budget) * 100);
    } else {
      budgetUtilization = 24; // Default/Mock
    }
    ...
    return [
      ...
      { subject: 'Team', A: memberWorkload.length > 0 ? 85 : 0, fullMark: 100 },
    ];
  ...
  // 9. Resource Utilization (Mocked capacity for now)
  const resourceUtilizationData = useMemo(() => {
    return memberWorkload.map(member => {
      return {
        name: member.name,
        load: Math.min(100, Math.round((member.taskCount / 10) * 100)), // 10 tasks as 100% load
        capacity: 100
      };
    });
  }, [memberWorkload]);
  ...
  const financialData = useMemo(() => {
    ...
    const cost = (totalMinutes / 60) * (project.hourly_rate || 50);
  ...
  const predictiveFinancialForecastData = useMemo(() => {
    ...
    const avgDailyBurn = totalSpent > 0 ? totalSpent / Math.max(1, (now.getTime() - new Date(project.created_at).getTime()) / (1000 * 60 * 60 * 24)) : 100;
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-12 items-center">
            <div className="lg:col-span-1 space-y-6">
               ...
               <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest max-w-xs leading-relaxed">
                 Using deep learning models to predict project trajectories based on current velocity and historical team performance metrics.
               </p>
            </div>
            <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="p-8 rounded-[3rem] bg-background/40 backdrop-blur-xl border border-border/40 hover:border-primary/40 transition-all duration-500">
                  ...
                  <p className="text-2xl font-black text-foreground mb-2">Alpha Release</p>
                  <p className="text-[10px] font-black text-primary uppercase tracking-widest">ETA: {predictiveMetrics.daysToFinish !== Infinity ? Math.round(predictiveMetrics.daysToFinish * 0.4) : '??'} Days</p>
               </div>
               <div className="p-8 rounded-[3rem] bg-background/40 backdrop-blur-xl border border-border/40 hover:border-primary/40 transition-all duration-500">
                  <div className="flex items-center gap-4 mb-6">
                  ...
                  <p className="text-2xl font-black text-foreground mb-2">Steady Growth</p>
                  <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest">+12% Velocity vs Last Sprint</p>
               </div>
               <div className="p-8 rounded-[3rem] bg-background/40 backdrop-blur-xl border border-border/40 hover:border-primary/40 transition-all duration-500">
                  <div className="flex items-center gap-4 mb-6">
                  ...
                  <p className="text-2xl font-black text-foreground mb-2">High Precision</p>
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">{predictiveMetrics.confidence}% Variance Accuracy</p>
               </div>
               <div className="p-8 rounded-[3rem] bg-background/40 backdrop-blur-xl border border-border/40 hover:border-primary/40 transition-all duration-500">
                  <div className="flex items-center gap-4 mb-6">
                  ...
                  <p className="text-2xl font-black text-foreground mb-2">Minimal Friction</p>
                  <p className="text-[10px] font-black text-sky-500 uppercase tracking-widest">Low Bottleneck Probability</p>
               </div>
            </div>
          </div>
        </div>
