...
import { 
  ...
  SkipBack, SkipForward, Subtitles, Settings, Gauge, Check, AlertCircle 
} from 'lucide-react';
...
export function VideoPlayer({ 
    ...
    if (src.includes('.m3u8')) {
      if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (event, data) => {
           if (data.fatal) { setIsBuffering(false); setHasError(true); }
        });
  ...
  const handleVideoClick = (e: ReactMouseEvent<HTMLVideoElement>) => {
    if (e.detail === 1) {
        ...
        if (clickX > rect.width / 2) videoRef.current.currentTime += 10; 
      ...
      {!hasError && (
          <div className={cn(
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="relative flex-1 flex items-center group/scrubber h-4">
                <input 
                  type="range" min="0" max="100" value={progress || 0} onChange={handleSeek}
                  className="w-full h-1.5 bg-white/20 rounded-full appearance-none accent-primary hover:h-2.5 transition-all duration-300 z-10 relative shadow-[0_0_10px_hsl(var(--primary)_/_0.5)]" 
                />
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-5">
                ...
                <button onClick={onPrevious} disabled={!onPrevious} className={cn("transition-colors", onPrevious ? "text-white/80 hover:text-white" : "text-white/20 cursor-not-allowed")} title="Previous">
                  <SkipBack className="h-5 w-5 fill-current" />
                </button>

                <button onClick={togglePlay} className="text-white hover:text-primary transition-all drop-shadow-[0_0_8px_hsl(var(--primary)/0.5)] mx-1 transform hover:scale-110" title={isPlaying ? "Pause (Space)" : "Play (Space)"}>
                  {isPlaying ? <Pause className="h-8 w-8 fill-current" /> : <Play className="h-8 w-8 fill-current" />}
                </button>

                <button onClick={onNext} disabled={!onNext} className={cn("transition-colors", onNext ? "text-white/80 hover:text-white" : "text-white/20 cursor-not-allowed")} title="Next">
                  <SkipForward className="h-5 w-5 fill-current" />
                </button>

                <div className="flex items-center gap-2 group/vol ml-4">
                  <button onClick={toggleMute} className="text-white hover:text-primary transition-colors" title="Mute (m)">
              ...
              <div className="flex items-center gap-5 relative">
                ...
                {localSubtitles.length > 0 && (
                  <div className="relative">
                    <button onClick={() => handleMenuToggle('cc')} className={cn("transition-colors", activeSubtitle !== -1 ? "text-primary drop-shadow-[0_0_8px_hsl(var(--primary)/0.6)]" : "text-white hover:text-white/80")} title="Subtitles/CC">
                    ...
                    {showCC && (
                      <div className="absolute bottom-full right-0 mb-4 w-44 bg-background/90 backdrop-blur-2xl border border-border/50 rounded-2xl shadow-2xl p-2 flex flex-col gap-1 z-50 animate-in slide-in-from-bottom-2">
                        ...
                        <button onClick={() => { setActiveSubtitle(-1); setShowCC(false); }} className="flex items-center justify-between px-3 py-2 text-xs font-bold text-foreground hover:bg-muted/80 rounded-xl transition-colors">
                          Off {activeSubtitle === -1 && <Check className="h-4 w-4 text-primary" />}
                        </button>
                        {localSubtitles.map((sub, idx) => (
                          <button key={idx} onClick={() => { setActiveSubtitle(idx); setShowCC(false); }} className="flex items-center justify-between px-3 py-2 text-xs font-bold text-foreground hover:bg-muted/80 rounded-xl transition-colors">
                ...
                {(qualityLevels.length > 0 || !src.includes('.m3u8')) && (
                  <div className="relative">
                    <button onClick={() => handleMenuToggle('quality')} className={cn("transition-colors flex items-center gap-1", activeQuality !== -1 ? "text-primary drop-shadow-[0_0_8px_hsl(var(--primary)/0.6)]" : "text-white hover:text-white/80")} title="Settings">
                    ...
                    {showQuality && (
                      <div className="absolute bottom-full right-0 mb-4 w-44 bg-background/90 backdrop-blur-2xl border border-border/50 rounded-2xl shadow-2xl p-2 flex flex-col gap-1 z-50 animate-in slide-in-from-bottom-2">
                        ...
                        {src.includes('.m3u8') ? (
                            <>
                                <button onClick={() => changeQuality(-1)} className="flex items-center justify-between px-3 py-2 text-xs font-bold text-foreground hover:bg-muted/80 rounded-xl transition-colors">
                                  Auto {activeQuality === -1 && <Check className="h-4 w-4 text-primary" />}
                                </button>
                                {qualityLevels.map((level, idx) => (
                                  <button key={idx} onClick={() => changeQuality(idx)} className="flex items-center justify-between px-3 py-2 text-xs font-bold text-foreground hover:bg-muted/80 rounded-xl transition-colors">
                                    {level.label || `${level.height}p`} {activeQuality === idx && <Check className="h-4 w-4 text-primary" />}
                                  </button>
                                ))}
                            </>
                        ) : (
                            <button className="flex items-center justify-between px-3 py-2 text-xs font-bold text-foreground hover:bg-muted/80 rounded-xl transition-colors opacity-70 cursor-default">
                ...
                <div className="relative">
                  <button onClick={() => handleMenuToggle('speed')} className={cn("transition-colors", playbackRate !== 1 ? "text-primary drop-shadow-[0_0_8px_hsl(var(--primary)/0.6)]" : "text-white hover:text-white/80")} title="Playback Speed">
                  ...
                  {showSpeed && (
                    <div className="absolute bottom-full right-0 mb-4 w-40 bg-background/90 backdrop-blur-2xl border border-border/50 rounded-2xl shadow-2xl p-2 flex flex-col gap-1 z-50 animate-in slide-in-from-bottom-2">
                      ...
                      {[0.5, 1, 1.25, 1.5, 2].map((rate) => (
                        <button key={rate} onClick={() => changePlaybackRate(rate)} className="flex items-center justify-between px-3 py-2 text-xs font-bold text-foreground hover:bg-muted/80 rounded-xl transition-colors">
                ...
                <button onClick={toggleFullscreen} className="text-white hover:text-primary transition-transform hover:scale-110" title="Fullscreen (f)">
