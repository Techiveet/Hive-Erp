...
use App\Mail\TenantStatusChanged;
use App\Mail\AdminStatusChanged;
use App\Mail\AdminCredentialsUpdated;
...
class TenantController extends Controller implements HasMiddleware
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $tenant = new Tenant();
            $tenant->id = strtolower($request->id);
            $tenant->name = $request->name;
            $tenant->plan = $request->plan;
            $tenant->is_active = true;
            $tenant->admin_email = strtolower($request->admin_email);
            $tenant->save();
            ...
            $tenant->run(function () use ($request) {
                ...
                try {
                    ...
                    \Illuminate\Support\Facades\Log::warning("Node provisioned, but welcome email failed: " . $mailEx->getMessage());
                }
            });

            // 🚀 FIX: Logged under Tenant Management
            activity('Tenant Management')->causedBy(auth()->user())->event('created')
            ...
            // 🚀 FIX: Logged under Tenant Management
            activity('Tenant Management')->causedBy(auth()->user())->event('failed')
